...
**本プロジェクトの最上位テーマ（不可侵）。** ユーザーは自然言語で指示するだけでよい。コマンド実行・スクリプト起動・ファイル編集・git 操作は **すべて Claude Code が代行** する。

- ユーザーに「`bash ...` を実行してください」「`python3 ...` してください」「このファイルを編集してください」と **依頼してはならない**（CP-6 Ⅲ・`user-confirmation-minimization.md` §4）。
- 唯一の例外は、GitHub / claude.ai の **アカウント権限が物理的に必要な Web 操作**（テンプレ作成・PAT 発行・claude.ai 設定・課金 = A-6 相当）だけ。
- **セットアップすら例外でない**: 運用リポで「セットアップして」と言えば、Claude が bootstrap・ミッション記入・初期コミット → PR を代行する（`/onboarding`・`docs/setup/onboarding.md`）。
- 「ローカルで実行してください」と言いたくなったら、まずクラウドで Claude が自律実行できないか必ず検証する（`user-confirmation-minimization.md` §4・L-077）。
...
| Wiki | `wiki/` | **Claude** | LLM 生成。`index.md`（目次）+ `log.md`（追記ログ） |
| Inbox | `ideas/` `bookmarks/` | 人間が投稿 → Claude が整理 | 気軽な入力の受け皿 |

タスクは GitHub Issues + Labels で管理する。Claude が ingest / query / lint の 3 操作で wiki を維持する。
**操作の詳細・不変条件は `docs/rules/wiki-operations.md`（`.claude/rules/` 常駐）が SSOT。**
...
定期実行は Claude Code ネイティブのルーティン機能で回す（gh-aw は使わない・`docs/automation/routines.md`）。

インターフェースが Claude Code のみであることは、上記「絶対条件」と一体である: **ユーザーは Claude に話しかけるだけ** で、セットアップ（`/onboarding`）から日常使い・定期自動化（`/routines-setup`）まで Claude が実装・スクリプト実行を代行する。ユーザーにターミナル操作を求める設計・ドキュメントは作らない。
...
| `no-github-actions.md` | **GitHub Actions 不使用ポリシー**: CI を Claude Code ハーネス（self-reviewer のワークツリー隔離 e2e・フック・ルーティン）で代替する SSOT |
| `claude-wiki-hub-sync-rules.md` | **公開テンプレート自律同期**: main マージ後に `publish-template.sh` を確認なしで自律実行する SSOT |
...
`save-metadata-index.md`（保存メタデータ・横断インデックスの生成/検索/集計 SSOT・#24）/
...
- **GitHub 操作の primary は公式 GitHub MCP（`mcp__github__*`）とトークン直叩き**（`GH_TOKEN` のみで動作）。
  PR・Issue・クロスリポ操作（`github_sync_repo.py` の Trees API 等）はすべて MCP/REST で完結する。
- クラウドで生存するのは: 非 repo スコープ REST（`gh api user` / `gh search`）と **git 操作**（`git clone https://...` / `git fetch/pull/push`・git プロキシは別系統）のみ。
- クラウドで 403 になる（= MCP へ切替）: `gh issue/pr list`・`gh repo view`・`gh api repos/{o}/{r}/...`・`gh api graphql`・`gh repo clone`。urllib で `api.github.com` を直叩きしても同じプロキシを通るため **同じ 403**（フォールバックにならない）。
- **gh CLI はハーネス内部の任意アクセラレータ**。ユーザーのセットアップ対象ではない（ユーザーが設定するのは
  PAT = `GH_TOKEN` 1 個だけ。gh のインストール・`gh auth login` は不要）。クラウドで gh が無い/失敗しても
  MCP・`gh_vars.py`（変数取得）に自動フォールバックする（`env-vars.md`・Issue #40）。
- ローカル実行（`gh` が直接 GitHub に到達できる環境）で gh CLI を使うとき: リポ指定に `-R kai-kou/claude-wiki-hub`、`gh pr create` に `--head {現在のブランチ}` `--base main`
  を付与する（プロキシ環境対策）。gh 不在時の代替表は `github-mcp-fallback-patterns.md`。
