...
検出条件:
  1. Open状態のPR（kai-kou/claude-wiki-hub）
...
# bootstrap.sh が REPO の kai-kou/claude-wiki-hub を置換すれば OWNER/REPO_NAME も追従する。
...
def _validate_repo() -> None:
    if not OWNER or not REPO_NAME or "kai-kou" in REPO or "claude-wiki-hub" in REPO:
        print(
            f"ERROR: REPO の形式が不正です: '{REPO}'（owner/name 形式が必要。"
            "bootstrap.sh で kai-kou/claude-wiki-hub を置換してください）",
            file=sys.stderr,
        )
        sys.exit(2)
