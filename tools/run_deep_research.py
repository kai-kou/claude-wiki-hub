...
def _repo_slug() -> str:
    ...
    `kai-kou/claude-wiki-hub` を返す（プロジェクト適用時に bootstrap で置換する）。
    """
    return os.environ.get("GITHUB_REPOSITORY") or "kai-kou/claude-wiki-hub"
...
def run_diy(research_id: str, theme: str, prompt_text: str) -> RunResult:
    """DIY フォールバック実装（Sonnet 4.6 + WebSearch / WebFetch）。
    ...
    result.cost_usd = 0.55  # 想定単価（Sonnet 4.6 60K入力+25K出力）
