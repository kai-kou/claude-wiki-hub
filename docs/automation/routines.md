...
- 設定: `claude.ai/code/routines`（Web UI）または対話セッションで `/schedule`。
- 既定でブランチは `claude/` プレフィックスのみに push。
- 例: 「毎週日曜 20:00 に wiki を lint し、矛盾・孤立ページを是正する PR を出す」

> **⚠️ 確定事実（公式 docs + 実機確認・2026-06-27）: Claude Code on the web セッション（本リポで動く Claude）からはルーティンを追加・更新できない。** web セッション内では `/schedule` が無効化され、作成/更新 API も存在しない（公開 API は既存ルーティンを発火する `/fire` のみ）。ルーティン設定は **ユーザー自身が web UI / Desktop / 非 web CLI の `/schedule`** で行う必要がある（個人 claude.ai アカウントに属するため・A-6 相当）。
> ユーザー向けの詳細セットアップ手順 + 本プロジェクト推奨ルーティンのコピペ定義は `docs/setup/routines-setup.md`（`/routines-setup` で Claude が対話提示）。「ルーティンを設定して」と言われたら代行できると誤認せず、本ガイドで案内する。
...
> **web の Routines は「1 つだけ」**（薄いディスパッチャ）にし、何を・いつ動かすかは
> リポジトリ内の cron テーブル `config/routine_jobs.yaml` で一元管理する。定期実行の追加・変更は
> YAML を編集して PR するだけで完結し、**web のルーティン設定を変更しなくて済む**。
> 実行プロトコル・セットアップは `docs/automation/routine-dispatch.md` / `docs/setup/routines-setup.md`（SSOT）。

単一ルーティン（毎日 1 回）が毎回 `python3 tools/routine_scheduler.py --due` を実行し、いま due なジョブだけを走らせる。初期ジョブ:

1. **週次 wiki lint**（日曜・Sonnet）: 矛盾・孤立ページ・データギャップを是正 → PR（`wiki-operations.md`）。
2. **定期リサーチ**（水曜・Opus）: 関心トピックを research-runner でリサーチ → `wiki/` に ingest。
3. **リポジトリ監視**（平日・Haiku）: Issue/PR/コミットを監視 → 取りこぼしを `ideas/` or Issue 化。
