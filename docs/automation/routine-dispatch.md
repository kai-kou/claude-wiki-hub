...
> web の Routines を **1 つだけ**（薄いディスパッチャ）にし、「何を・いつ動かすか」はリポジトリ内の
> cron テーブル `config/routine_jobs.yaml` で一元管理するための実行プロトコル。
> 目的: 定期実行の追加・変更のたびに **web のルーティン設定を変更しなくて済む** ようにする。
...
- **ジョブの追加・変更・停止 = `config/routine_jobs.yaml` を編集して PR するだけ**（web 設定は不変）。
- web ルーティンのプロンプトには **具体的なタスクを書かない**（下記の定型のみ）。

---

## 1. web ルーティンに貼る定型プロンプト（これだけ・更新不要）
...
- **`window_hours` は web ルーティンの実行間隔に一致させる**。`config/routine_jobs.yaml` の `window_hours` で設定（既定 24）。
- **推奨運用 = 日次ルーティン（window 24h）**:
  - web ルーティンを **1 日 1 回**（例: 毎日 20:00）に設定する。
  - 各ジョブの cron の **日・月・曜日フィールドが「どの日に走るか」** を決める。
    例: `0 20 * * 0`（日曜）→ 日曜を含む直近 24h の日次実行で 1 回だけ due。
  - cron の **時・分は実行日選択には影響するが、実際の実行時刻は web ルーティンの単一発火時刻** になる
    （時刻の精密制御が要らない定期メンテ向き）。各 cron 発火は **ちょうど 1 回** due になる。
- **毎時運用（window 1h）にしたい場合**:
  - `window_hours: 1` にして、web ルーティンを **毎時 cron**（`/schedule update` で設定）にする。
...
> web 変更は不要。1 日複数回など細かい時刻が要るときだけ、ユーザーが **1 度だけ** web の cron と
...
```yaml
window_hours: 24            # due 判定窓（= web ルーティンの間隔）
jobs:
  - id: <一意ID>            # 必須
...
| ドキュメント | 関係 |
|------------|------|
| `docs/setup/routines-setup.md` | ユーザー向け単一ルーティンのセットアップ手順 |
