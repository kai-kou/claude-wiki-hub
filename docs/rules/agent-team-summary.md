...
| メインセッション・実装・PR対応 | Sonnet 4.6 (`claude-sonnet-4-6`) | `/effort medium` |
| 調査・検証・チェック（サブエージェント） | Haiku 4.5 (`claude-haiku-4-5`) | — |

**原則**: 迷ったら Sonnet。Opus は知能重視タスク（長文生成・複雑設計）など明確に必要な場合のみ。

> **`.claude/settings.json` の `model` 設定について**: メインセッションの既定は `claude-sonnet-4-6`（標準作業のコスト最適）。上表のとおり知能重視タスク（長文生成・複雑設計等）は Opus 4.8 を `/model` ・ `/effort xhigh` で **明示指定** する運用で、常時 Opus は高コストのため既定にはしない。両者の差は意図的（モデル運用の詳細は `docs/rules/claude-code-optimization.md` を参照）。
