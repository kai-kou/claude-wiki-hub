...
| 長時間パイプライン | 5 ステップごとに `/compact` を検討。未コミット作業は区切りで小まめにコミットして保護 |
...
> **圧縮タイミングは Claude 標準に委ねる**: 以前は `CLAUDE_CODE_AUTO_COMPACT_WINDOW` で実効コンテキストを縮めて早期圧縮させる緩和策を挙げていたが、圧縮タイミングを制御する env・フックは撤廃した（ユーザー明示決定）。本ベースは Claude Code 標準の Auto Compaction（約 95% 到達時の自動圧縮）に完全に委ねる。パースエラーは上記の運用対策（`/clear`・Sonnet 切替・ツール数抑制）で対処する。
...
| `PreCompact` | **コンテキスト圧縮前**（v2.1.105 新設） | ⬜ 未利用（圧縮タイミングは Claude 標準 Auto Compaction に委ねる） |
| `PostCompact` | コンテキスト圧縮後 | ⬜ 未利用（同上・pre/post-compact.sh は撤廃済み） |
...
圧縮タイミングを制御する env・フック（旧 `pre-compact.sh` / `post-compact.sh`）は撤廃し、Claude Code 標準の Auto Compaction（約 95% 到達時の自動圧縮）に完全に委ねる方針（ユーザー明示決定）。`PreCompact` / `PostCompact` は **プラットフォーム機能としては実在** するが本ベースでは採用しない。

- 未コミット作業の保全は `Stop`（stop-router.sh）の WIP 自動コミットが担う。
- L-100 の SessionStart クリーンアップ対策（圧縮後の新セッション起動でローカル変更が巻き戻る問題）は、圧縮前の一次防御フックを持たないため「区切りで小まめにコミットする」運用でカバーする。
...
#### 3. PreCompact / PostCompact フックは不採用（圧縮タイミングは Claude 標準に委ねる）

圧縮タイミングを制御する env・フックは撤廃済み（ユーザー明示決定）。圧縮は Claude Code 標準の Auto Compaction（約 95% 到達時の自動圧縮）に委ね、未コミット作業の保全は `Stop`（stop-router.sh）の WIP 自動コミットと「区切りで小まめにコミットする」運用でカバーする（CC-BUG-14 の防御も同方針）。
