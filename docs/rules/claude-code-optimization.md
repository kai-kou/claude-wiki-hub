...
Opus 4.8 / Opus 4.7 / Opus 4.6 / Sonnet 4.6 に搭載。クエリの複雑さに基づき、内部で思考量を自動調整する。
...
| `/effort medium` | `"medium"` | バランス・適度なトークン節約 | 通常の実装・PR 対応（Sonnet 4.6 に特に推奨） |
...
- Sonnet 4.6 で通常作業: `/effort medium` でコスト削減
...
response = client.beta.messages.create(
    model="claude-sonnet-4-6",   # Executor: Sonnet（低コスト実行）
...
|--------|---------|---------|---------|
| 台本生成（Phase 3） | Sonnet 4.6 | Opus 4.8 | 品質向上、コスト削減（Opus 4.8 単独より安価） |
...
| 画像パイプライン | Sonnet 4.6 でデザイン定義書 + 全 visual_cue を一括処理（圧縮リスク低減） |
| レビュー・検証 | Haiku でコンテキスト効率最大化（不要な情報を渡さない） |
| 長時間パイプライン | 5 ステップごとに `/compact` を検討。Stop フックが未コミット変更を自動コミットで保護 |
...
| 高負荷でない工程は Sonnet 4.6 | `/model` | Sonnet 4.6 ではほぼ再現しない（台本生成など Opus 必須工程のみスポット使用） |
| 1 ターンのツール呼び出しを 8 個以下 | `session-safety-rules.md` | 連続 Bash 呼び出しで誘発されやすいため緩和 |

> **圧縮タイミングについて**: 本ベースはコンテキスト圧縮を Claude Code 標準の Auto Compaction（約 95% 到達時の自動圧縮）に委ね、`CLAUDE_CODE_AUTO_COMPACT_WINDOW` 等の env で圧縮タイミングを固定しない（#65）。任意の閾値で圧縮ウィンドウを狭めるとパイプライン中の圧縮過多を招くため、標準挙動を既定とする。圧縮専用フック（PreCompact/PostCompact）も持たず、未コミット作業の保護は Stop フックで行う。
...
| `PreCompact` | **コンテキスト圧縮前**（v2.1.105 新設） | ⬜ 未利用（標準の Auto Compaction に委ねる・#65） |
| `PostCompact` | コンテキスト圧縮後 | ⬜ 未利用（標準の Auto Compaction に委ねる・#65） |
...
|-----------|------|----------------------|
| `hookSpecificOutput.additionalContext` | 返した文字列を Claude のコンテキストに注入 | `gh issue list --label status:in-progress/waiting-claude` の結果を注入しセッション再開プロトコル Step1〜4 を自動化 |
...
#### PreCompact フックの設計パターン（v2.1.105+・参考例）

> 以下は将来 `PreCompact` を導入する場合の **擬似コード例** であり、本ベースにこのファイルは存在しない（圧縮系フックは未採用・#65）。

```bash
#!/bin/bash
# 参考例（本ベースには存在しない）: .claude/hooks/pre-compact.sh — 圧縮前処理
...
> **現在の方針**: 圧縮系フック（`PreCompact` / `PostCompact`）は未採用。未コミット保護は `Stop` フックで実施し、コンテキスト圧縮は標準の Auto Compaction に委ねる（#65）。上記は将来 `PreCompact` を導入する場合の参考例であり、本ベースでは有効化しない。
> **実装時の絶対制約**: trigger='auto' は無条件で通過（exit 0）。これを守らないとコンテキスト上限到達時に API エラーが表面化してリクエスト失敗が起きる（きなこ技術監修 2026-04-18）。
...
> **本プロジェクトでの判断（きなこ技術監修 2026-04-18）**: 現時点では **使用しない**。Gemini Code Assist + GitHub Copilot の2つの外部 AI レビュアーと self-reviewer スキルで品質担保は十分。収益化（YPP Stage 1）達成後に費用対効果を再評価する。
...
> **本プロジェクトへの影響**: 長時間パイプライン（audio/image/video）で圧縮後の再起動コストが削減される。OAuth 401 ループ修正により、スケジュールタスクの認証エラーが減少する見込み。
...
| 対策 | 実装状況 | 効果 |
|------|---------|------|
| Stop フック自動コミット | ✅ 実装済み | 未コミット作業およびセッション終了時の保護（圧縮系フックは未採用・#65） |
...
**現在の運用（2026-07-11 更新）**: クラウド（Routines）は Claude が claude-code-remote MCP（`create_trigger` / `update_trigger` 等）でセッション内から代行管理する（SSOT: `docs/setup/routines-setup.md`）。`/schedule` は web セッションでは使えない（ローカル CLI のみ）。
...
| `/compact` の戦略的使用 | 5 ステップ検証ポイントで手動圧縮を検討 |
...
**本プロジェクトの防御策**:
- ✅ Stop フックで自動コミット（`stop-router.sh`）
- ✅ Stop フックで Slack 通知（`stop-slack-notify.sh`）
- ✅ Issue コメントへの再開ステップ記録（`session-safety-rules.md`）
- **追加推奨**: 長時間パイプラインは 10 ステップを超えたら手動 `/compact` を検討
...
**本プロジェクトの防御策**:
- ✅ Stop フックで自動コミット（作業データは保護済み）
- **追加推奨**: Auto-Compact に頼らず、パイプラインの自然な区切り（Phase 遷移時等）で手動 `/compact` を実行する。手動圧縮のほうが孤立ブロックのリスクが低い
...
**影響**: 本プロジェクトは **12 個のフック** を使用（SessionStart ×2, PermissionRequest ×1, PreToolUse ×4, PostToolUse ×1, PostToolUseFailure ×1, Stop ×3）。コンテキスト肥大化のリスクあり。
**本プロジェクトの防御策**:
...
Layer 4: コンテキスト管理
  ...
  → Auto-Compact に頼らず、Phase 遷移時に手動 `/compact` を実行（CC-BUG-14）
...
| **基本コマンド** | `ant messages create --model claude-sonnet-4-6 --max-tokens 1024 -p "Hello"` |
| **ストリーミング** | `ant messages stream --model claude-sonnet-4-6 -p "長い応答"` |
...
> **2026-06-05 実装ステータス（Issue #2615・ユーザー「すべて対応」指示／最終）**: ✅ **適用**= P-2 / P-3 / P-5（@kinako のみ）/ P-7 プルーニング監査 / P-8。⏸️ **見送り**= P-1（致命的・自律実行停止）/ P-4（既存フック+CI で充足・プラグインはスループット害）。🟡 **部分**= P-6（additionalContext は既存等価・JSON 全面書換は高リスクで見送り）/ P-7 @import 分割（report-only 提案に留め自動適用せず）。判断基準は「正確性＝既存の自律ワークフローを壊さない」を最優先（ユーザーの併存指示）。
...
| P-5 | `@kinako`/`@mochi` に `memory: project` 付与 | セリフパターン・指摘履歴の蓄積でキャラ監修精度向上 | ✅ **@kinako のみ適用**。`memory: project`（`.claude/agent-memory/kinako/MEMORY.md` を seed）。`memory` は公式仕様で Read/Write/Edit を自動有効化するため、**@mochi はITオンチ設定（高度ツール不使用）維持のため除外**。保護対象（frontmatter `tools`・方言禁止）は不変。CLAUDE.md の編集ツール禁止ルールにメモリ自己記録の carve-out を明記 |
...
> 2026-06-06 ユーザー指示で再リサーチ（最新機能・運用BP の2エージェント並列）。P-1〜P-8 と Dynamic Workflows Epic（#2586）に **含まれない新差分**。@kinako（技術裏取り）+ @owner（破壊リスク予測）で議論し、ユーザーが「全部Issue化して段階導入」を選択。**1提案=1PR** で進行中（追跡: #2672）。

| # | 提案 | 統合判定 | ステータス / 根拠 |
|---|------|---------|------------------|
| P-9 | `ENABLE_PROMPT_CACHING_1H=1`（プロンプトキャッシュ TTL 5分→1h） | ✅ 即採用（最優先） | ✅ **適用**（GitHub Variable 設定済 + env-vars.md 追記）。@kinako/@owner 両GO・低リスク。回帰: 次パイプライン1サイクルでコスト計測 |
| P-10 | headless `--fallback-model`（`claude -p` のモデル降格） | ✅ 採用 | ✅ **適用**（2026-06-06・#2672）。`run_deep_research_workflow.py`（`--fallback-model claude-sonnet-4-6`）/ `run_discussion_review.py` / `monitor_x_mentions.py` / `monitor_qiita_comments.py`（各 `--fallback-model claude-haiku-4-5`）に追加。⚠️ settings.json 配列指定は未実装（#8413）・`-p` フラグ限定・overload(529)のみ発火 |
