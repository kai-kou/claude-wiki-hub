...
| 長時間パイプライン | 5 ステップごとに `/compact` を検討。PostCompact フックが自動コミットで保護 |

#### tool call パースエラーの緩和（`CLAUDE_CODE_AUTO_COMPACT_WINDOW`・2026-06-06 追加）
...
| 対策 | 設定箇所 | 効果 |
|------|---------|------|
| `CLAUDE_CODE_AUTO_COMPACT_WINDOW=400000`（任意・既定では無効） | `.claude/settings.json` の `env` | 実効コンテキストを 400K に縮め早期圧縮させる。1M の streaming バグ誘発を抑制（公式環境変数・値を実上限より高くしても無効）。圧縮タイミングを固定したくない場合は設定しない |
...
> **既定で無効にしている理由**: 圧縮タイミングを env で固定すると、パイプライン中の圧縮過多や任意の閾値での強制圧縮が起きうる。本ベースは Auto Compaction（約 95% 到達時の自動圧縮）に委ね、env での明示指定はしない方針（2026-06-27 に env から削除）。tool call パースエラーが頻発する環境でのみ上記を追加して有効化する。
> **注意**: 環境変数は **startup 時に読み込まれる** ため、反映は次回 `claude` 起動時から。圧縮は PostCompact フックが自動コミットで保護するため、早期圧縮による作業ロスのリスクは低い。悪影響（パイプライン中の圧縮過多）が確認された場合は値を `512000` に緩めるか、env から削除して即時無効化できる。
