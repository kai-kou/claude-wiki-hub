...
| `timeout * python3 *tools/*.py` | `timeout 30s python3 tools/check_pending_pr_reviews.py --json` |
...
| `gemini-image-mcp-server.kinamocchi-tech.workers.dev` | Gemini Image MCP サーバー |
...
| `youtube-api-proxy.kinamocchi-tech.workers.dev` | YouTube API プロキシ（Cloudflare Workers） |
...
| `tools/check_pending_pr_reviews.py` | `api.github.com` | レビュー待ち PR 検出（project-sync） |
