...
**症状**: `git push` だけが 403/413/502 で失敗する（pull/fetch/gh は動く）。クラウドのプロキシが書き込みをブロックするため。

**フォールバック順**: ① `mcp__github__push_files`（GitHub MCP）→ ② `tools/github_push_helper.py`（Contents API base64 PUT）。ファイル単位 push なのでマージコミットは作れない。
**クロスリポ書き込み**: プロキシは PAT 直叩き（埋め込みトークン git push / gh REST / urllib REST）を全拒否し、セッションの GitHub App 認証のみ許可する。別リポへは ① `add_repo` でスコープ追加 → ② 埋め込みトークンなしのプレーン git push または MCP `push_files`。「403 = トークン権限不足」と即断せず、まず add_repo 漏れを疑う。
...
**症状**: セッション再開時の `session-start.sh` の `git reset`/`checkout -- .`/`clean -fd`、または Bash から起動した `claude -p`（headless）の子セッションが同じクリーンアップを走らせ、未コミット・未追跡ファイルを破壊する。

**対策**: 大量編集中はファイル作成のたびに即コミット＆リモート反映する。`claude -p` はcwd=一時ディレクトリで起動する（project フックを読ませない）。session-start.sh は headless 起動検知でクリーンアップをスキップする（本ベース実装済み）。
...
「予測」して本文に書き、それを観測事実として推論を続けてしまう（例: 未取得の CI/レビュー結果を
「APPROVE→マージ完了」と捏造、存在しないファイルの中身を Read もせず捏造、辻褄合わせでさらに捏造）。
根本原因はツール呼び出し（自分の行為）とツール結果（システムの観測）の混同。
...
## L-114: クラウドで gh の repo スコープ操作（REST + GraphQL）が 403 でブロックされ MCP が唯一経路

**症状**: クラウド実行環境で `gh` は存在するのに、選択中リポジトリへの repo スコープ REST・GraphQL 操作
（`gh issue/pr list`・`gh repo view`・`gh api repos/{o}/{r}/...`・`gh api graphql`・`gh repo clone`）が
egress プロキシに 403 でブロックされる（2026-06-30 実機検証・Issue #121）。`urllib` で API 直叩きしても
**同一プロキシを通るため同じ 403**（フォールバックにならない）。

**対策**: repo スコープの GitHub 操作は **公式 GitHub MCP（`mcp__github__*`）を一次経路** にする。
生存するのは非 repo REST（`gh api user` 等）と git 操作（`git clone/fetch/pull/push`＝別系統プロキシ）のみ。
代替表・検証マトリクスの SSOT は `docs/rules/github-mcp-fallback-patterns.md`。
**判定基準**: `gh` の repo 操作が 403 → 即 MCP へ切替。
