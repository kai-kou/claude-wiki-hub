...
別リポへ書くには ① `add_repo` でそのリポをセッションスコープに追加 → ② **埋め込みトークンを使わないプレーン git push**
（プロキシが App 認証を注入）または **MCP `mcp__github__push_files`**。`github_sync_repo.py`（urllib+PAT）は
...
✅ コンテキストが肥大化する前に区切りで /clear・新規セッションに切り替える（圧縮は Claude 標準の Auto Compaction に委ねる）
...
選択中リポジトリへの操作だけが egress プロキシに 403 でブロックされる（2026-06-30 実機検証・Issue #121）:
...
`resolve_review_thread` 等）。生存するのは非 repo REST（`gh api user`・`gh search`）と git 操作
（`git clone https://...`・`git fetch/pull/push`＝git プロキシは別系統）のみ。代替表・検証マトリクスの
SSOT は `docs/rules/github-mcp-fallback-patterns.md`。
**判定基準**: `gh` の repo 操作が 403（GraphQL proxying / connect GitHub App）→ 即 MCP へ切替。
