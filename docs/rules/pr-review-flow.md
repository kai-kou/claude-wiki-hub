...
>   --pr-url "https://github.com/kai-kou/claude-wiki-hub/pull/{N}" \
...
| 変更内容 | 理由 |
|---------|------|
| BGM WAV ファイルの変更 | 末尾ギャップ（0.3秒）が意図的なパディングと分かる |
| LFS ポインターが含まれる PR | フォールバックロジックが組み込み済みと分かる |
| `speedScale: 1.1` の設定変更 | 意図的な調整値と分かる |
| `completed_steps: []` リセット | 意図的な動作（L-037）と分かる |
| `[wip]` コミットを含む PR | セッション保護用と分かる |
| `--limit 1000`（gh CLI） | Issue 数が 200 件超のため必要と分かる |
| 台本系 PR（`content/scripts/V*_script.json` 変更） | 下記「台本PR共通定型文」を貼り付け |
| audio-pipeline 系 PR（`content/scripts/V*_timed.json` 変更時） | 下記「audio-pipeline PR 向け定型文」を貼り付け |
| meta.yaml PR（`content/meta/V*_meta.yaml` 変更時） | 下記「meta.yaml 変更時の設計意図」を貼り付け |

### 台本PR共通定型文（コピペ用・推奨）

`content/scripts/V*_script.json` を変更する PR では、以下の定型文を「設計意図・既知の警告」セクションに貼り付けると、Gemini/Copilot の頻出誤指摘（emotion/action/voicevox_params/fact_check_flags）を一括で抑止できる。

```markdown
## 設計意図・既知の警告（AIレビュアー向け）

### 台本JSON 共通の意図的な設計（指摘不要）
- `emotion: "annoyed"` / `"angry"` / `"secret"` 等が含まれる場合: 有効値は `normal/neutral/excited/confused/sad/whisper/tsukkomi/teaching/gentle` のみで、それ以外は `tools/check_golden.py` (Lv4 CI) が自動検出してエラーにする。AIレビュー側で重ねて指摘する必要はない
- `action` 値が `glasses_down` / `explaining` / `wing_spread` 等の場合: 英語直書きの有効値は `jump/shake/nod/spin/tilt_head/groom/flap/idle/null` のみで、Lv4 CI が検出済み。日本語フリーテキスト（"メガネを外す"等）は許容
- `dialect:"osaka"` 行で `voicevox_params` が欠落: `check_osaka_voicevox_params()` (Lv4 CI) が自動検出。標準値は `{speedScale: 1.2, intonationScale: 1.3, pitchScale: 0.02}` ±0.1
- `voicevox_speaker_id` と `voicevox_style` の対応: もっちー `12=ふつう/32=わーい/33=びくびく/34=おこ/35=びえーん`、きなこ `2=ノーマル/0=あまあま/6=ツンツン/4=セクシー/36=ささやき/37=ヒソヒソ`。`check_speaker_style_consistency()` (Lv4 CI) が不整合を検出
- `fact_check_flags` の `source_url: null`: URL未確定の中間状態を表す **意図的な許容値**。`null` 値の指摘は不要。ただしキー自体の欠落はバグ
```

> **効果**: このセクションがある PR では Gemini / Copilot が同じ指摘を繰り返す確率が大幅に低下する。
> `.gemini/styleguide.md` と `.github/copilot-instructions.md` にも反映済みのため、
> リポジトリレベルで一度学習した指摘については将来的にセクション不要になる可能性がある。

### audio-pipeline PR 向け定型文（`V*_timed.json` 変更時）

`content/scripts/V*_timed.json` を変更する PR では、以下を「設計意図・既知の警告」セクションに追記する。

```markdown
### timed.json の意図的な設計（指摘不要）
- `bgm_tracks` フィールドが `timed.json` に存在する場合: `validate_timed_json.py` で必須項目として検証される、設計上不可欠なフィールド。`generate_audio.py` が（Shorts 等で）自動出力しない場合があるため audio-pipeline 内で手動追加している。「不要なフィールド」や「スクリプトとの不整合」として指摘しないこと
- `audio_file` パスが `audio/` で始まる（`assets/` なし）: Remotion が `Config.setPublicDir("../assets")` を設定しているため `assets/` を付けると二重パスになる。仕様
- `voicevox_speaker_id: 12`（`timed.json`）: 白上虎太郎 ふつう（speaker ID 12）は正しい値
```

### meta.yaml 変更時の設計意図（`content/meta/V*_meta.yaml` 変更時）

`content/meta/V*_meta.yaml` を変更する PR では、以下を「設計意図・既知の警告」セクションに追記する。

```markdown
### meta.yaml の意図的な設計（指摘不要）
- `localized.en.tags` が含まれない: YouTube Data API v3 の `localizations` エンドポイントは `title`・`description` のみ対応であり、`tags` は非対応。タグは最上位の `tags` フィールドのみで設定する。「localized.en に tags がない」という指摘は不要（Issue #1813）
```
...
2. 【必須・L-050】PR作成直後の存在確認（L-050 対策）
   gh pr list --head {現在のブランチ名} -R kai-kou/claude-wiki-hub \
...
```
# ① PRイベント購読を開始
mcp__github__subscribe_pr_activity(owner="kai-kou", repo="claude-wiki-hub", pull_number={pr_number})
...
- **記録の SSOT は PR スレッド返信・Resolve・Issue コメントであって、チャットではない**。チャットは揮発し圧縮・セッション再開で失われる。レビュー指摘・対応・検証結果は必ず PR 側に残す。
...
3. **指摘コメントへの返信（必須）**: 対応完了・スキップいずれの場合も、該当コメントスレッドに返信を投稿し、会話をResolve済みにする
   ...
   - 返信API: `gh api repos/{owner}/{repo}/pulls/{pr_number}/comments/{comment_id}/replies --method POST -f body="..."`
   - インラインコメントのthread resolve: GraphQL `resolveReviewThread` mutation（後述）。Step 1 で取得したインラインコメントの `id` と、GraphQL `reviewThreads` の `comments.nodes[0].id` を照合して対応する `thread_id` を特定します。
...
```bash
# GitHub MCP ツールでマージ
mcp__github__merge_pull_request(owner="kai-kou", repo="claude-wiki-hub", pull_number=N, merge_method="squash")
```
...
1. outdated レビューコメントを全て Resolve する
   # GraphQL で全スレッドの thread_id を取得してから resolveReviewThread を実行
   gh api graphql -f query='
     query {
       repository(owner: "kai-kou", name: "claude-wiki-hub") {
         pullRequest(number: {pr_number}) {
           reviewThreads(first: 100) {
             nodes { id isResolved }
           }
...
3. `gh pr create` でPR作成（`--head` / `--base` / `Closes #N` を必ず含める）
4. 【必須・L-050】PR作成直後に存在確認: `gh pr list --head {ブランチ名} -R kai-kou/claude-wiki-hub --limit 1 --json number,url,state --jq '.[0] | select(.url != null) | "PR #\(.number) \(.state): \(.url)"'` → URL を確認してから報告する
