...
> （2026-06・Issue #2860・inline 指摘 9,565 件 / 889 PR、うち AI レビュアー指摘 5,821 件）から
...
>
> severity / AIレビュアー判定ロジックは前者が SSOT で、後者は import 再利用する。主指標「指摘ゼロPR率」の分母は **週内マージ全PR数**（指摘ゼロPR も含む）であり、`prs_with_comments`（指摘付きPRのみ）を分母にしない（比率が下がらず無意味になるため・@kinako 指摘）。
...
| ☐ | リポジトリ名・パス・コマンドの typo（claude-wiki-hub の末尾 i 重複等） | 機械 | 実指摘あり（誤操作リスク） |
...
| ☐ | キャラ口調: もっちーに京都弁・きなこに大阪弁/です・ます調が混入していないか | 目視（Lv3 フック併用） | 80 件 |
...
| ☐ | `gh issue/pr list` の `--limit` 不足 | **機械** | 100 件（P-12） |
...
| ☐ | gh コマンドは `-R kai-kou/claude-wiki-hub` 統一（`--repo` 不可） | **機械** | CLAUDE.md 規約 |
