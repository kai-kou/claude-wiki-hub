...
> Hot 化する場合は `.claude/rules-extra.conf` にファイル名を 1 行追加して `./tools/check_rules_sync.sh --fix` を
> 実行する（conf は upstream 同期で消えない・#73。`ESSENTIAL_RULES` 直接追記は同期で上書きされるため非推奨）。
...
| self-reviewer | self-review-checklist.md, script-rules.md, research-rules.md |
| retrospective | retrospective-rules.md, self-review-checklist.md |
| refinement | refinement-rules.md, research-rules.md |
| pr-review-watcher | self-review-checklist.md |
...
| workflow-health-check | youtube-content-variation-rules.md, self-review-checklist.md |
| retro-try-handler | self-review-checklist.md |
...
コンテキスト圧縮は Claude Code 標準の Auto Compaction（コンテキストウィンドウが約 95% に達した時の自動圧縮）に委ねる。圧縮タイミングを制御する env（`CLAUDE_CODE_AUTO_COMPACT_WINDOW`）や圧縮専用フックは持たない（#65）。
