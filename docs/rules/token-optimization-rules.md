...
コンテキスト圧縮は Claude Code 標準の Auto Compaction（約 95% 到達時の自動圧縮）に委ね、圧縮タイミングを制御する env（`CLAUDE_CODE_AUTO_COMPACT_WINDOW` 等）は設定しない。
一方、Token blowout（1 セッションでの出力トークン暴走）を防ぐために、圧縮とは別機構の以下のトークン累計ガードを実装している。
