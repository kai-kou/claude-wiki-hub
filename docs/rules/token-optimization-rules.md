...
## 実行中エンフォースメント（Issue #98）

`CLAUDE_CODE_AUTO_COMPACT_WINDOW`（任意設定・本ベースの既定では無効）は閾値到達時に **圧縮してセッションを継続** する設定であり、停止しない。必要なら `.claude/settings.json` の `env` に追加して有効化する（詳細は `claude-code-optimization.md`）。
Token blowout を防ぐために、以下の追加エンフォースメントを実装した。

| 設定 | 値 | 動作 |
|------|----|------|
| `CLAUDE_CODE_TOKEN_LIMIT_WARN` | 350,000 | PostToolUse 後に警告メッセージを stderr に注入（Claude のコンテキストへ）し、コミット & プッシュを促す |
| `CLAUDE_CODE_TOKEN_LIMIT_HARD` | 380,000 | PostToolUse 後に強制停止メッセージを stderr に注入し exit 2 で停止する |

実装: `.claude/hooks/token-budget-guard.sh`（`post-tool-use-validate.sh` から呼び出し）。
セッション JSONL の `assistant.message.usage.output_tokens` を合算してトークン数を測定する。
JSONL が見つからない・jq が利用不可の場合はフェイルオープン（exit 0）で継続する。
