...
個人 wiki を育てる日常利用（ファイル編集・コミット）は **トークン設定なしで完結** する（[公式](https://code.claude.com/docs/en/claude-code-on-the-web)）。
...
| GitHub Repository Variables 経由の secrets 一元管理（Slack/R2 等） | 変数値の読み取りに `repo`/`actions` 権限のトークンが要る（`gh_vars.py`）。App も MCP もこれをカバーしない。クラウドセッションに専用 secrets ストアは無い（[公式](https://code.claude.com/docs/en/claude-code-on-the-web)） |
| クロスリポ操作（メンテナの公開テンプレ同期） | App トークンは接続リポ中心で、別リポ書き込みは PAT/REST が要る |
...
> この区分は[ルーティン定期実行](../automation/routines.md#ルーティンの-github-認証と-pat-要否正本ssot)でも同じ（無人実行でも PR を開くまでは PAT 不要・自動マージと Issue 化は PAT 必要）。

### GH_TOKEN を登録したときの自動ロード（自動化層）

PAT を登録すると、他の env は **GitHub Repository Variables** に置いて一元管理できる。
`session-start.sh` が取得は **トークン（`GH_TOKEN`）だけで完結** する2系統で自動ロードする:

1. **`tools/gh_vars.py`（urllib・トークンのみ・gh CLI 不要）** — primary。GitHub REST API を直接叩く。
2. **`gh variable list`（gh CLI）** — あれば高速に使う任意アクセラレータ（gh があれば ①、無ければ ② に自動フォールバック・Issue #40）。

`gh` CLI のインストール・`gh auth login` はユーザー作業ではない（ハーネスが自前で試み、失敗しても ② で継続）。
...
```bash
gh variable set SLACK_BOT_TOKEN -R kai-kou/claude-wiki-hub --body "xoxb-..."
gh variable set SLACK_CHANNEL_ID -R kai-kou/claude-wiki-hub --body "C0XXXXXXXXX"
# ... 残りの変数
```
...
```
GitHub Variables: loaded 28 var(s) via gh_vars.py
```

## 新しい環境変数が必要になった時のフロー

1. Claude Code が「新しい環境変数 `XXX` が必要です」と判断する
2. Claude Code が自律的に GitHub Variables に設定する:

```bash
gh variable set XXX -R kai-kou/claude-wiki-hub --body "値"
```

3. 次回セッションから自動的に利用可能になる
4. 現セッションで即時利用が必要な場合は `export XXX=YYY` で一時設定する

**ユーザーへの手動設定依頼は不要**（`GH_TOKEN` を除く）。
...
# gh CLI で直接操作
gh variable get SLACK_BOT_TOKEN -R kai-kou/claude-wiki-hub
gh variable set SLACK_BOT_TOKEN -R kai-kou/claude-wiki-hub --body "xoxb-..."
gh variable delete SLACK_BOT_TOKEN -R kai-kou/claude-wiki-hub
```
...
| 変数名 | 用途 | 必須 | 値の例 |
|--------|------|------|--------|
| `GH_TOKEN` | GitHub パーソナルアクセストークン（GitHub MCP 認証 + Variables 読み取り + gh CLI 認証）。**標準構成では不要**・自動化層（Issue/PR 自動化・Slack/R2・クロスリポ）を使うときだけ設定する任意のアップグレード | △ 任意 | `ghp_...` |
...
| `GCP_PROJECT` | GCP プロジェクトID（BGM生成） | BGM生成時 | `kinakomocchi` |
...
| `X_OWNER_HANDLE` | X 予算枯渇時に『おやつ代やばい』@mention 通知を飛ばす飼い主ハンドル（@なし・未設定時は既定 `k_aik_ou`・空文字で X 通知を無効化）。`x_budget_tracker.py check --x-notify` が使用（Issue #2424） | △（手動課金の気づき通知） | `k_aik_ou` |
...
| `R2_BUCKET_NAME` | R2 バケット名（デフォルト: `claude-wiki-hub-videos`） | 任意 | `claude-wiki-hub-videos` |
| `R2_PUBLIC_DOMAIN` | R2 パブリックバケットのカスタムドメイン（未設定時は r2.dev ドメイン） | 任意 | `videos.kinamocchi-tech.workers.dev` |

### Claude Code 最適化変数

| 変数名 | 用途 | 必須 | 値の例 |
|--------|------|------|--------|
| `ENABLE_PROMPT_CACHING_1H` | Anthropic API プロンプトキャッシュの TTL を 5分→1時間に延長。毎時パイプラインで CLAUDE.md・常駐ルール群を繰り返しロードするため、キャッシュ読込の90%割引でコスト削減（5分超のターン間隔が常態のため確実に元が取れる）。Claude Code BP差分 P-9（#2672・@kinako/@owner 専門チーム GO） | 推奨 | `1` |
...
セッション開始
  ↓
GH_TOKEN を取得（Claude.ai 環境変数 or settings.local.json）
  ↓
変数取得（GH_TOKEN だけで完結・gh CLI の有無に依存しない・Issue #40）:
  ├─ ① gh CLI あり  → gh variable list -R <repo> --json name,value
  └─ ② gh CLI なし/失敗/0件 → tools/gh_vars.py（urllib・トークンのみ）にフォールバック
  ↓
各変数について:
  ├─ 既にプロセス環境変数に存在する → スキップ（Claude.ai 側の値を優先）
  └─ 未設定 → export + CLAUDE_ENV_FILE に書き出し（セッション全体に伝搬）
  ↓
ログ出力: "GitHub Variables: loaded N var(s) via {gh|gh_vars.py}"
```

### 優先順位

1. **Claude.ai 環境変数設定**（プロセス環境変数として既に存在） → 最優先
2. **GitHub Repository Variables**（session-start.sh で取得） → フォールバック
...
- `gh variable list` を実行して変数値を **対話的に stdout 表示** する（平文流出）。変数取り込みのみの用途（session-start.sh 等）は例外
