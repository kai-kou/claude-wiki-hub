...
- **task 層（GitHub Issue）**: Issues は GitHub 側で検索・集計できるため、現フェーズはインデックス対象外（YAGNI）。
  ※ idea / bookmark の Issue 管理化（#77）後も本原則は不変: `kind:idea` / `kind:bookmark` Issue は
  **ライフサイクル管理層** であり、本文・メタデータの真実源は従来どおりファイル（frontmatter）。
  インデックスはファイル経由で idea/bookmark を対象に含むため、Issue を直接スキャンする必要はない。
