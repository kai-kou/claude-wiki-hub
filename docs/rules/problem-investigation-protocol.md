...
[ ] GitHub Repository Variables に登録済みか確認:
    gh variable list -R kai-kou/claude-wiki-hub | grep {VAR}
...
[ ] gh variable set で Claude が自律的に設定可能か確認（CLAUDE.md より）
[ ] 自己解決不可の場合のみ、必要な値・取得手順をユーザーに提示
```

**典型的な自己解決アクション**:
- `gh variable set XXX_TOKEN -b "..."` で設定（公開可能な値の場合）
...
```
[ ] gh run view {run_id} --log でフルログ取得
