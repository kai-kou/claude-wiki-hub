...
> **Warm 層（既定でタスク依存・E-B #20）**: Scheduled Tasks 等でマルチセッションを並行運用するプロジェクトのみ Hot 化する（`.claude/rules-extra.conf` に 1 行追加 → `./tools/check_rules_sync.sh --fix`。conf は upstream 同期で消えない・#73）。単一セッション運用では Warm のままでよい。
...
PR 作成ステップの直前:
  1. gh pr list で同一対象のオープン PR を再チェック
