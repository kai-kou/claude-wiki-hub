...
評価観点:
  ...
  - セルフレビュー学習ログ（self-review-checklist.md）への追記候補
```
...
Try Issue を一覧で取得するには以下のコマンドを使用する:
...
# Claude 担当の Try のみ
gh issue list -R kai-kou/claude-wiki-hub --label "type:retro-try" --label "assignee:claude" --state open

# 特定パイプラインの Try のみ（タイトル検索）
gh issue list -R kai-kou/claude-wiki-hub --label "type:retro-try" --search "[Retro][script]"

# 高優先度の Try のみ
gh issue list -R kai-kou/claude-wiki-hub --label "type:retro-try" --label "priority:high" --state open
```
...
### 次のアクション
- Try Issue の対応: `gh issue list -R kai-kou/claude-wiki-hub --label "type:retro-try" --state open`
```
