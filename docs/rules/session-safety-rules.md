...
`.claude/hooks/token-budget-guard.sh` が PostToolUse 後に出力トークン累計を監視する。

| 閾値 | 設定 | 動作 |
|------|------|------|
| WARN（350K） | `CLAUDE_CODE_TOKEN_LIMIT_WARN` | 警告を stderr に注入（コンテキストへ）・コミット & プッシュを促す |
| HARD（380K） | `CLAUDE_CODE_TOKEN_LIMIT_HARD` | 強制停止メッセージを stderr に注入・exit 2 |

WARN 到達時は即座に残作業をコミット & プッシュして次セッションに引き継ぐ。
