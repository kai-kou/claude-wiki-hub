...
```bash
# 1. PR 作成
gh pr create --head {branch} --base main -R kai-kou/claude-wiki-hub --title "..." --body "..."

# 2. 【必須・L-050】PR 存在確認
gh pr list --head {branch} -R kai-kou/claude-wiki-hub --limit 1 --json number,url,state \
...
mcp__github__subscribe_pr_activity(owner="kai-kou", repo="claude-wiki-hub", pull_number=N)
...
# 【main マージ後・必須】claude-wiki-hub 自律同期（claude-wiki-hub-sync-rules.md）
# ※ claude-wiki-hub 開発リポジトリのみ。運用フォークには publish-template.sh が存在しないためスキップされる
# 【クラウド必須前段】claude-wiki-hub はセッションスコープ外。先に add_repo で追加する:
#   mcp__claude-code-remote__add_repo(owner="kai-kou", repo="claude-wiki-hub")
# クラウドはプレーン git（プロキシ注入の GitHub App 認証）で push する（PAT 埋め込み・urllib は 403）。
# プレーン push が失敗したら MCP（mcp__github__push_files）で同期する（claude-wiki-hub-sync-rules.md §4）。
[ -f scripts/publish-template.sh ] && bash scripts/publish-template.sh
```
...
```bash
python3 tools/check_pending_pr_reviews.py --mine --actionable-only --json   # ① 自セッション作成PRを最優先回収（#47）
python3 tools/check_pending_pr_reviews.py --actionable-only --json          # ② 共有スコープの孤児PRを救済
```

`needs_prompt`→`/code-review` 実行→即マージ / `needs_response`→指摘対応 / `awaiting_review`→他セッション実行中（待機・介入しない・CP-4・L-109）。自スコープ優先・active_session 除外の詳細は `pr-review-flow.md`「セッション復帰」参照。
