...
| 26 | `PreCompact` | 圧縮開始前（matcher: manual/auto）。exit 2 で圧縮ブロック可 | ⬜ 未採用（標準の Auto Compaction に委ねる・#65） |
| 27 | `PostCompact` | 圧縮完了後（matcher: manual/auto）。decision 制御なし | ⬜ 未採用（標準の Auto Compaction に委ねる・#65） |
...
### UserPromptSubmit（user-prompt-submit-guard.sh）
- ユーザー入力の高リスクパターン（main 直 push / rm -rf / .env・秘密情報 / フック無効化 /
  settings.local.json への env 書き込み）を検出し、関連ガードレールを **助言としてコンテキスト注入** する
- 既定は **非ブロッキング**（exit 0）。誤検知で正当作業を止めないため。真に破壊的なパターンを
  ブロックしたいプロジェクト向けに exit 2 のブロック例をコメントで残してある

### 圧縮系フック（PreCompact / PostCompact）は未採用（#65）
- 圧縮タイミングの制御・圧縮前後の独自処理は行わず、**Claude Code 標準の Auto Compaction** に委ねる。
- 未コミット作業の保全は `Stop`（stop-router.sh）と手動コミット規律（`session-safety-rules.md`）でカバーする。
  圧縮専用の安全策フック（旧 `pre-compact.sh` / `post-compact.sh`）は撤廃済み。

## 4. 未採用イベントの採否理由（明示）

| イベント | 未採用の理由 |
|---------|------------|
| `SessionEnd` | WIP 保全は `Stop`（stop-router.sh）で既にカバー済み。logout 時のクリーンアップが必要なプロジェクトのみ追加する（settings.json に `SessionEnd` を追記）。汎用ベースでは冗長コミットを避けるため未採用 |
...
| `PostToolBatch` / `InstructionsLoaded` 他 | 現行ハーネスの守備範囲（main 直 push 防止・PR フロー）に必要十分。将来要件が出たら本表の採否を更新する |
