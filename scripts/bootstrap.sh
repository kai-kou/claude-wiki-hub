...
#
# 役割:
#   1. プレースホルダ置換（kai-kou/claude-wiki-hub, kai-kou/claude-wiki-hub, {{PROJECT_NAME}} 等）
...
for f in "${FILES[@]}"; do
  # bootstrap.sh 自身 / sync-upstream.sh は置換しない
  # sync-upstream.sh の UPSTREAM_URL がユーザーの repo に書き換わるのを防ぐ
  [ "$f" = "${BASH_SOURCE[0]}" ] && continue
  [[ "$f" == */scripts/sync-upstream.sh ]] && continue
  sed -i \
    -e "s#kai-kou/claude-wiki-hub#${ESC_SLUG}#g" \
    -e "s#kai-kou/claude-wiki-hub#${ESC_SLUG}#g" \
    -e "s#{{PROJECT_NAME}}#${ESC_NAME}#g" \
    -e "s#{{PROJECT_DESCRIPTION}}#${ESC_DESC}#g" \
    "$f" 2>/dev/null || true
done
# kai-kou / claude-wiki-hub 単独（slug 置換後に残るもの）を個別に置換
for f in "${FILES[@]}"; do
  [ "$f" = "${BASH_SOURCE[0]}" ] && continue
  [[ "$f" == */scripts/sync-upstream.sh ]] && continue
  sed -i \
    -e "s#kai-kou#${ESC_OWNER}#g" \
    -e "s#claude-wiki-hub#${ESC_REPO}#g" \
    "$f" 2>/dev/null || true
done
