...
#
# 取り込み対象（ハーネス層）:
#   .claude/hooks/  .claude/skills/  .claude/agents/  .claude/output-styles/  .claude/commands/
#   docs/rules/  docs/setup/  docs/automation/  tools/  scripts/  .github/  .gitignore  requirements.txt
#
# 取り込まない（データ層 + プロジェクト固有ファイル）:
#   raw/  wiki/  ideas/  bookmarks/  content/  docs/project-mission.md
#   config/routine_jobs.yaml（運用リポがジョブをカスタマイズする前提のため上書きしない）
#
...
# 初回シード: config/routine_jobs.yaml は運用リポのカスタマイズ前提のため同期上書きしないが、
# 旧テンプレート由来のリポには存在しないため「無い場合だけ」upstream から 1 度シードする
# （ディスパッチャルーティンが routine_scheduler.py --due で必要とする・#93）。
if [ ! -f "config/routine_jobs.yaml" ]; then
  if git checkout "$UPSTREAM_REF" -- "config/routine_jobs.yaml" 2>/dev/null; then
    echo "[sync-upstream] config/routine_jobs.yaml が無かったため upstream からシードしました（以後は同期対象外・自由に編集可）"
  fi
fi

# slug 再置換: bootstrap.sh がリポ固有 slug に置換したファイル（.claude/ docs/ tools/ scripts/ 等）を
# upstream から上書きすると、テンプレートの slug に巻き戻る（/status /next 等が別リポを参照して静かに壊れる）。
# origin の slug を導出し、upstream slug を再置換して bootstrap 直後と同じ状態に戻す（#93）。
UPSTREAM_SLUG="${UPSTREAM_URL#*github.com/}"; UPSTREAM_SLUG="${UPSTREAM_SLUG%.git}"
ORIGIN_URL="$(git remote get-url origin 2>/dev/null || true)"
OWN_SLUG="${ORIGIN_URL#*github.com[:/]}"; OWN_SLUG="${OWN_SLUG%.git}"
if [ -n "$OWN_SLUG" ] && [ -n "$UPSTREAM_SLUG" ] && [ "$OWN_SLUG" != "$UPSTREAM_SLUG" ] && [[ "$OWN_SLUG" == */* ]]; then
  echo "[sync-upstream] slug 再置換: $UPSTREAM_SLUG → $OWN_SLUG"
  UP_OWNER="${UPSTREAM_SLUG%%/*}"; UP_REPO="${UPSTREAM_SLUG##*/}"
  OWN_OWNER="${OWN_SLUG%%/*}";     OWN_REPO="${OWN_SLUG##*/}"
  _esc() { printf '%s' "$1" | sed -e 's/[\\&#]/\\&/g'; }
  while IFS= read -r -d '' f; do
    # 自分自身（UPSTREAM_URL を保持）は置換しない（bootstrap.sh と同じ除外）
    [[ "$f" == */scripts/sync-upstream.sh ]] && continue
    sed -i \
      -e "s#$(_esc "$UPSTREAM_SLUG")#$(_esc "$OWN_SLUG")#g" \
      -e "s#$(_esc "$UP_REPO")#$(_esc "$OWN_REPO")#g" \
      -e "s#$(_esc "$UP_OWNER")#$(_esc "$OWN_OWNER")#g" \
      "$f" 2>/dev/null || true
  done < <(find "${HARNESS_PATHS[@]}" -type f \( -name '*.md' -o -name '*.py' -o -name '*.sh' -o -name '*.json' -o -name '*.yaml' -o -name '*.yml' -o -name '*.txt' \) -print0 2>/dev/null)
fi
