...
#### ハーネスが古いフォークで「アップデートを取り込んで」が誤動作する場合

古いハーネス（2026-07-10 以前のテンプレートから作成）では、この発話が誤って `apply-base`（claude-code-base 適用）に振られることがあります。その場合は Claude に **明示的に** こう依頼してください:

> 「`bash scripts/sync-upstream.sh --yes` を実行してください」

スキルルーティングを迂回して upstream（テンプレートリポジトリ）の最新ハーネスを直接取り込みます。**一度取り込めば、以降は通常の「アップデートを取り込んで」が正しく動作します。**（補助: 「claude-wiki-hub からアップデートを取り込んで」と取り込み元を名指すのも有効ですが、確実なのは上記のスクリプト明示です）
