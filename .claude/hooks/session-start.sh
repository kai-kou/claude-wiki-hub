...
# 対象リポジトリ（bootstrap.sh が kai-kou/claude-wiki-hub を置換、または PROJECT_REPO で上書き）
REPO="${PROJECT_REPO:-kai-kou/claude-wiki-hub}"
...
# クラウドの network policy で gh インストールがブロックされても、GH_TOKEN さえあれば
# ② で確実にロードする（GitHub 操作の primary はトークン/MCP・gh はハーネス内部の任意アクセラレータ）。
if [ -n "${GH_TOKEN:-}" ]; then
...
# 失敗（exit≠0）を `|| true` で握りつぶすと「[rules-sync] OK」と誤表示され、余剰 symlink
# （Hot 層肥大化・Haiku 200k 超過の原因）が温存されたまま気づけない無音失敗になる
# （haiku-context-overflow-followup 議論・Critical-1）。セッション起動は止めず、失敗時は
# 明示的に警告する。
