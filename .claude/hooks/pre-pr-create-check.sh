...
#
# 対象ツール:
#   - Bash の `gh pr create`（ローカル / gh が通る環境）
#   - `mcp__github__create_pull_request`（gh pr create が proxy 403 で失敗する
#     クラウド環境の主経路。matcher 外だと Layer 0 ゲートを完全素通りするため対応・再発防止）
# 未コミット・未push のファイルがあれば PR 作成をブロックする。
# PR 作成が成立するときは Layer 1 セルフレビュー（FAIR・全PR必須）のリマインダーを出す（非ブロッキング）。
...
if [ "$tool_name" = "mcp__github__create_pull_request" ]; then
  # MCP 経由の PR 作成（クラウド主経路）。コマンド文字列を持たないため直接ゲートへ。
  is_pr_create=1
elif [ "$tool_name" = "Bash" ]; then
  command=$(echo "$input" | jq -r '.tool_input.command // ""')
  # gh pr create なら後段ゲートへ。それ以外（poll_pr_reviews 等）は下のブロックで個別処理。
...
if [ "$tool_name" = "Bash" ] && echo "${command:-}" | grep -qE 'poll_pr_reviews\.sh'; then
