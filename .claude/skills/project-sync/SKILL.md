...
> **実行モデル**: 本フローは2層で構成される。
> - **`tools/sync_project.py`（Python 実装）**: Step 1（エンティティ ID 採番・採番運用時のみ）・Step 2（{バックログ分類}の waiting-user 除去）・Step 3（Projects V2 Assignee 同期）
> - **Claude が gh / mcp で直接実行するスキル手順**: Step 0・2.5・2.6・3.5・3.7・3.9・3.95 等（ラベル操作・auto-close・誤分類リセット・レポート出力）
>
> Step 2.5 / 2.6 は `sync_project.py` の関数ではなく、Claude が SKILL.md に従い gh CLI / GitHub MCP で実行する手順である（コードへの実装は将来の拡張候補）。
...
Step 1: エンティティ ID 採番（採番運用するプロジェクトのみ・未採用ならスキップ）
  ...
  ├─ 全 Issue から最大 ID 番号を取得
  ├─ 連番で ID を採番
  └─ Issue タイトルを "[{エンティティID}] {ワークフローフェーズ}: {概要}" に更新

Step 2: {バックログ分類}ラベル修正
  ├─ {バックログ分類}（例: [ネタ候補] タイトル or phase:1-* ラベル）の Issue を取得
...
```
python3 tools/sync_project.py
```

### ドライラン（確認のみ）

```
python3 tools/sync_project.py --dry-run
```

### スクリプト実行後の後続処理（手動実行時のみ）

sync_project.py で新たにエンティティ ID が採番された Issue がある場合:
1. 出力の「採番結果サマリー」を確認する
