...
`mcp__github__add_issue_comment` を使用（`owner: "kai-kou"` / `repo: "claude-wiki-hub"` / `issue_number: {既存Issueの番号}`）。
...
*フィルタ: `gh issue list -R kai-kou/claude-wiki-hub --label "type:retro-try" --state open`*
```
