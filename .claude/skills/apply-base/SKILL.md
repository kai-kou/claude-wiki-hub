---
name: apply-base
description: kai-kou/claude-code-base（汎用自律運用ベース・全リポの祖）の内容を現在のリポジトリへ反映（適用・再同期）する。**「claude-code-base」または「ベース（＝claude-code-base の意）」を明示的に名指した発話のときだけ** 使用する（例:「claude-code-base の内容を反映して」「claude-code-base を適用して」「ベースを反映して」「claude-code-base で初期化して」）。⚠️ 除外（本スキルを使わない）:「アップデートを取り込んで」「更新して」「最新を取り込んで」「ハーネスを更新して」等の **取り込み元を名指さない汎用的な更新依頼** は本スキルではなく、運用リポの upstream 追従（`sync-upstream` スキル・`scripts/sync-upstream.sh`・upstream = claude-wiki-hub）を使う。private リポジトリのベースを git clone（クラウドでは gh の repo スコープ操作が 403 でブロックされるため git/MCP 経路）で取得して適用するため、ユーザーがコマンドを打つ必要はない。
---
...
# 🔴 リポ文脈ゲート: wiki-hub 運用フォークでの誤起動だけを止める（#87 再発防止・二重防衛）
#   判定は 2 信号の AND: 「sync-upstream.sh を持つ（= wiki-hub 系で upstream 追従経路がある）」
#   かつ「publish-template.sh を持たない（= dev リポ claude-wiki-hub ではない）」= 運用フォーク。
#   この 1 条件でのみブロックすることで、apply-base の正当な用途（claude-code-base を任意の
#   既存リポへ後付け適用/初期化・claude-code-base 自身の自己適用）を誤ブロックしない。
if [ -f "scripts/sync-upstream.sh" ] && [ ! -f "scripts/publish-template.sh" ]; then
  echo "このリポジトリは wiki-hub 運用フォーク（operational fork・upstream=claude-wiki-hub）です。claude-code-base の適用対象ではありません。" >&2
  echo "アップデートの取り込みは「アップデートを取り込んで」と話しかけるか、bash scripts/sync-upstream.sh を実行してください。" >&2
  exit 1
fi
...
> **なぜ 2 信号 AND なのか**: `publish-template.sh` 単独の「無ければブロック」判定だと、apply-base の正当な用途
> （claude-code-base を **任意の既存リポへ後付け適用/初期化**・claude-code-base 自身の自己適用）まで誤ブロックする
> （それらのリポは publish-template.sh を持たない）。ブロックすべきは **wiki-hub 運用フォークだけ** なので、
> 「upstream 追従経路 `sync-upstream.sh` を持つ（= wiki-hub 系）」AND「dev リポの印 `publish-template.sh` を持たない」
> の 2 条件が揃ったときに限定する。
>
> | リポ | sync-upstream.sh | publish-template.sh | ゲート |
> |---|---|---|---|
> | dev（claude-wiki-hub） | 有 | 有 | 通過（apply-base 可） |
> | wiki-hub 運用フォーク | 有 | 無 | **ブロック → sync-upstream へ案内** |
> | claude-code-base 自身 / 任意の後付け適用先 | 無 | 無 | 通過（apply-base 可） |
>
> 補助信号（必要時の確認用）: `scripts/sync-upstream.sh` の `UPSTREAM_URL`（フォークでは claude-wiki-hub に固定）・`git remote`。
...
- 🔴 **タスク実行モードによっては `git ls-remote` 自体が 403 になる（L-115・詳細は `docs/rules/lessons/cross-repo.md`）**: GitHub Issue/PR 対応のリモートタスク（システムプロンプトに「Repository Scope」が単一リポジトリで明示される形態）では、`mcp__claude-code-remote__add_repo` が提供されず、クロスリポ git 操作自体が遮断される（GH_TOKEN/ネットワーク設定の問題ではない）。この場合は GH_TOKEN 依頼に進まず、「このタスク実行モードでは claude-code-base に到達できない。`claude-code-base を反映して` を通常の claude.ai/code セッション（チャット形式）で実行してほしい」とユーザーへ案内し、本タスクでの適用は断念する。
