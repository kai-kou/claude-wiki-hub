...
description: Deep Research を完全自動実行するスキル。ネイティブ `/deep-research`（クラウド環境でも直接実行可能・Opus orchestrator）を主エンジン、Gemini Deep Research Max を第2、DIY（Sonnet 4.6 + WebSearch）を最終フォールバックとして、`content/research/{ID}_prompt.md` から `content/research/{ID}_deep_research.{md,json}` を自動生成し、品質ゲート・PR 作成・AIレビュー・自動マージまでを担う。「リサーチ自動化して」「ディープリサーチして」「research-runner」と依頼された時に使用する。
model: sonnet
effort: high
...
> 何でもよい。`kai-kou/claude-wiki-hub` はプロジェクト適用時に実リポジトリ slug に置換する
...
| 候補 | 実体 | いつ使うか |
|------|------|-----------|
| **`research-runner`（本スキル）** | `tools/run_deep_research_workflow.py`（`/deep-research` を直接実行・Opus orchestrator・adversarial 多票検証） | **既定**。ユーザーが「ディープリサーチ」を求めたら必ずこれを最初に起動する |
| ビルトイン `deep-research` | 自セッション内の WebSearch fan-out harness | 本スキルの **DIY フォールバック相当**。主エンジン・第 2 エンジンが失敗したときだけ（= Step 4） |
...
| 項目 | 内容 |
|---|---|
| **主エンジン** | **ネイティブ `/deep-research`（Dynamic Workflows・Opus orchestrator）** — `tools/run_deep_research_workflow.py`。正確性（adversarial 多票検証・一次ソース・未確証明示）が最高で、**必ず最初に実行する** |
| 第2エンジン | **Gemini Deep Research Max API** (`deep-research-max-preview-04-2026`)（主エンジンが真に失敗時） — `tools/run_deep_research_gemini.py` |
| フォールバック | **DIY**（Sonnet 4.6 + WebSearch + WebFetch）（上記2つが失敗時の最終手段） |
| コスト | **既定=サブスク週次枠経路（追加 $ ゼロ）**: セッション認証（Claude Code Max サブスク）をそのまま使用し（`DEEP_RESEARCH_USE_SUBSCRIPTION=1` 既定）、`/deep-research` は週次クォータの枠内で実行され追加課金なし。`DEEP_RESEARCH_USE_SUBSCRIPTION=0` で従来の API 従量経路に戻せる |
| 想定時間 | 本番では 30〜50分/本（wall-clock）。必ず `run_in_background` + heartbeat で監視する |
...
```bash
gh issue edit {ISSUE_NUM} -R kai-kou/claude-wiki-hub \
...
### Step 3: 主エンジン実行（ネイティブ `/deep-research`・Opus orchestrator）【必ず最初に実行】

> 🔴 **最重要**: 主エンジンは **ネイティブ `/deep-research`**（クラウド環境でも `claude -p` なしで直接実行可能）。
> これを **必ず最初に実行** する。Step 4 の DIY を最初から実行してはならない（安易な DIY 直行禁止）。
...
#   0=成功(schema OK) / 1=schema NG・normalize失敗 / 2=入力不足
...
- セキュリティ/動作: `--allowedTools` で事前許可（`tools/run_deep_research_workflow.py` 経由時）。**Bash/Write は Workflow のオーケストレーション実行に必須**。インジェクション対策は work_dir をリポジトリ外へ隔離 + 実行後に必ず破棄で緩和（git commit はサブプロセスにさせず親＝本スキルが行う。root では `--dangerously-skip-permissions` 不可）。
...
> モデルは既定 **Opus**（`DEFAULT_ENGINE_MODEL=claude-opus-4-8`）。コスト/時間削減が必要なら Gemini/DIY を使う。
...
```bash
SKIP=$(gh issue view {ISSUE_NUM} -R kai-kou/claude-wiki-hub --json labels \
...
if [ "$NEXT" -lt 3 ]; then
  ...
  gh label create "research-skip:${NEXT}" -R kai-kou/claude-wiki-hub --color FBCA04 2>/dev/null || true
  [ "$SKIP" -gt 0 ] && gh issue edit {ISSUE_NUM} -R kai-kou/claude-wiki-hub --remove-label "research-skip:${SKIP}"
  gh issue edit {ISSUE_NUM} -R kai-kou/claude-wiki-hub \
...
**完了判定**: EXIT=0 かつ `deep_research.{json,md}`（`engine=gemini-deep-research-max`）が揃ったら **Step 4 をスキップして Step 5 へ**。EXIT=1 → Step 4（DIY）。
...
### Step 4: 最終フォールバック（DIY・Sonnet 4.6 + WebSearch）【Step 3・3.5 両方失敗時のみ】
...
このスキル自身が Sonnet 4.6 として、本セッション内で WebSearch / WebFetch を実行し、
...
- `tools/run_deep_research_workflow.py` — 主エンジン（ネイティブ `/deep-research` の実行 + 正規化。内部は引き続き `claude -p` サブプロセスを使用）
