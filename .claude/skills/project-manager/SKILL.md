...
- `gh` CLI が認証済み（`gh auth status`）・`gh auth refresh -s project` で project スコープ付与済み
...
### PR 作成コマンド（プロキシ環境必須構文）

プロキシ環境では remote からブランチを自動検出できないため `--head` / `--base` を **必ず** 付与する:
...
```bash
# マイルストーン別進捗
gh api repos/kai-kou/claude-wiki-hub/milestones --jq '.[] | "\(.title): \(.closed_issues)/\(.open_issues + .closed_issues) done"'

# ステータス別の Issue 一覧
gh issue list --label "status:in-progress" --state open --repo kai-kou/claude-wiki-hub
gh issue list --state all --limit 100 --repo kai-kou/claude-wiki-hub

# Projects V2 ボード（使う場合）
gh project item-list PROJECT_NUMBER --owner kai-kou --format json
```
...
```bash
gh api repos/kai-kou/claude-wiki-hub/milestones --jq '.[] | "\(.number): \(.title)"'
gh api --method PATCH repos/kai-kou/claude-wiki-hub/milestones/MILESTONE_NUMBER -f state="closed"
```
