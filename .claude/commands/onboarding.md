...
6. 定期自動化（ルーティン）を 1 問だけ提案し（推奨 = 有効・スキップ可）、希望があれば `/routines-setup` の手順で
   Claude が `create_trigger`（claude-code-remote MCP）により単一ディスパッチャ（環境 Default・毎朝 8:00 JST）を
   **代行作成** する（`docs/setup/routines-setup.md`）。
7. CJK 整形・ルール symlink 同期 → `claude/` 作業ブランチにコミット（main 直 push 禁止・A-1）。マージは PAT 有無で分岐する:
...
8. `completion-report-rules.md` に従い「ご依頼の再掲 → 何ができるようになったか」を報告し、日常使いに誘導する。
